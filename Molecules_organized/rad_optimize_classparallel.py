...
# def write_comp(calc_energies1,calc_energies2):
...
#     f=open("comp3_new_%s.txt"%jobname,"w")
#     for molecule in hydrocbns:
#         f.write(f'{molecule}\n')
#         # for i, data in enumerate(hydcbn_data[molecule]):
#         f.write(f'{evtonm/np.array(hydcbn_data[molecule])}\n')
#         f.write(f'{calc_energies2[molecule]}\n')
        
#         # for i in range(len(hydcbn_data[molecule])):
#         #     f.write("\n%f\t%f"%(evtonm/hydcbn_data[molecule][i],calc_energies2[molecule][i]))
#         f.write("\n")

#     for molecule in heterocyls:
#         f.write(f'{molecule}\n')
#         # for i, data in enumerate(hetero_data[molecule]):
#         f.write(f'{evtonm/np.array(hetero_data[molecule])}\n')
#         f.write(f'{calc_energies2[molecule]}\n')
...
#     f.close()
#     return
def write_comp(d1_energies: dict, bright_energies: dict, jobname):
    exp_energies={'allyl': [3.07, 5.0], 'benzyl': [2.755548342, 3.999989529, 4.769218285], 'dpm': [2.370930696, 3.69046653], 'trityl': [2.405593703, 3.595990587], 'dpxm': [2.070794579, 3.310791333, 3.595990587], 'pdxm': [2.033594677, 3.02559208, 3.447190976], 'txm': [2.033594677, 2.951192275]}
    gmcqdpt_energies={'allyl': [3.4465528, 6.4685596], 'benzyl': [3.0465578,4.2007681, 5.2475845], 'dpm': [2.556545,4.1321081], 'trityl': [2.6301691, 3.9113602], 'dpxm': [2.3973264, 3.5094722, 3.9095689], 'pdxm': [2.3329004, 3.423308, 3.873217], 'txm': [2.303476, 3.3438274]}
    gmcqdpt_energies2={'allyl': [3.4465528, 6.4685596,6.06845868432759], 'benzyl': [3.0465578,4.2007681, 5.2475845,4.42133703101611], 'dpm': [2.556545,4.1321081,4.212428922], 'trityl': [2.6301691, 3.9113602,4.333618338287], 'dpxm': [2.3973264, 3.5094722, 3.9095689], 'pdxm': [2.3329004, 3.423308, 3.873217], 'txm': [2.303476, 3.3438274]}
    d1_file=open(f"compD1_{jobname}.txt","w")
    bright_file=open(f"compBrght_{jobname}.txt","w")
...
class ParameterOptimization:
    def __init__(self, params, weights, atom, jobname, normalise_weights, hydrocbns, heterocyls, hydcbn_data, hetero_data, opt, bounds):
        ...
        self.opt = opt
        # self.total_fit = 0
        # self.comp_energies2 = {}  # this is for heterocycles
        self.bds = bounds
        self.fit_iter = 0
        # self.manager = Manager()# using manager to create shared distionary between processor
        # self.Bright_energy = self.manager.dict()
        # self.D1_energy = self.manager.dict()  # this is for heterocycles
        self.iteration = 0
    def processor_info(self, file, params):
        with open(f'processor_log_{self.jobname}.txt', 'a') as f:
            f.write(f'\n{file} is using \n{params}\n')
            f.write(f'{file}{datetime.now()}\n')
    ...
    def process_hydrocarbons_parallel(self, file, active_params, total_fit, weights):
        ...
        if len(self.hydcbn_data[file]) == 1:  # when only D1 data is available
            fitness_inside+=self.process_hydrocarbons_only_d1(file, energies, oscs, s2_array, total_fit, weights, strng)

        # Process hydrocarbons with GMC and one bright state
        elif file in hydrocbns_w_gmc and file not in hydrocbns_w_2bright:
            fitness_inside+=self.process_hydrocarbons_with_gmc(file, energies, oscs, s2_array, total_fit, weights, strng)

        # Process with 2 bright states and has GMC
        elif file in hydrocbns_w_gmc and file in hydrocbns_w_2bright:
            fitness_inside+=self.process_hydrocarbons_with_two_bright_states(file, energies, oscs, s2_array, total_fit, weights, strng)

        # Process with two bright state but no GMC
        elif file in hydrocbns_w_2bright and file not in hydrocbns_w_gmc:
            fitness_inside+=self.process_hydrocarbons_with_two_bright_states_no_gmc(file, energies, oscs, total_fit, weights, strng)

        # Process with bright and d1
        elif len(self.hydcbn_data[file]) == 2:
            fitness_inside+=self.process_hydrocarbons(file, energies, oscs, total_fit, weights, strng)
    ...
    def process_heterocycles_parallel(self, file, active_params, total_fit, weights):
        ...
        if file in hetereo_only_d1:
            fitness_inside += self.process_heterocycles_with_only_d1(file, energies, total_fit, weights, strng)
        elif len(self.hetero_data[file]) == 2:
            fitness_inside +=  self.process_heterocycles_with_two_states(file, energies, oscs, total_fit, weights, strng)
        elif len(self.hetero_data[file]) == 3:
            fitness_inside += self.process_heterocycles_with_three_data(file, energies, oscs, total_fit, weights, strng)
        else:
            fitness_inside += self.process_heterocycles_full(file, energies, oscs, s2_array, total_fit, weights, strng)
        
        return fitness_inside
    
    
    # def process_hydrocarbons_batch(self, files, active_params, total_fit, weights):
    #     """ Process a batch of hydrocarbons in parallel using threads within a core """
    #     batch_fitness = 0
    #     with concurrent.futures.ThreadPoolExecutor(max_workers=10) as thread_executor:  # 10 threads per core
    #         futures = [thread_executor.submit(self.process_hydrocarbons_parallel, file, active_params, total_fit, weights) for file in files]
    #         batch_fitness = sum(f.result() for f in concurrent.futures.as_completed(futures))
    #     return batch_fitness

    # def process_heterocycles_batch(self, files, active_params, total_fit, weights):
    #     """ Process a batch of heterocycles in parallel using threads within a core """
    #     batch_fitness = 0
    #     with concurrent.futures.ThreadPoolExecutor(max_workers=20) as thread_executor:  # 20 threads per core
    #         futures = [thread_executor.submit(self.process_heterocycles_parallel, file, active_params, total_fit, weights) for file in files]
    #         batch_fitness = sum(f.result() for f in concurrent.futures.as_completed(futures))
    #     return batch_fitness    
    ...
    def fitness(self, active_params, inactive_params, weights, atom):
        ...
        # Split into batches of approximately 4 molecules per core
        hydrocarbon_batches = [self.hydrocbns[i:i + 4] for i in range(0, len(self.hydrocbns), 4)]
        heterocycle_batches = [self.heterocyls[i:i + 4] for i in range(0, len(self.heterocyls), 4)]

        # only process pooling
        # with concurrent.futures.ProcessPoolExecutor(max_workers= 16) as executor:
        #     # # Submit tasks for hydrocarbons in parallel
        #     # hydrocarbon_futures = [executor.submit(self.process_hydrocarbons_parallel, file, active_params, total_fit, weights) for file in self.hydrocbns]

        #     # # Submit tasks for heterocycles in parallel
        #     # heterocycle_futures = [executor.submit(self.process_heterocycles_parallel, file, active_params, total_fit, weights) for file in self.heterocyls]

        #     # for future in concurrent.futures.as_completed(hydrocarbon_futures + heterocycle_futures):
        #     #     result = future.result()  # Get the result of each future
        #     #     total_fit += result  # Accumulate the fitness results
        #     # Parallel processing hydrocarbons and heterocycles, returns results in parallel
            
        #     hydrocarbon_results = executor.map(self.process_hydrocarbons_parallel, self.hydrocbns, [active_params]*len(self.hydrocbns), [total_fit]*len(self.hydrocbns), [weights]*len(self.hydrocbns))
        #     heterocycle_results = executor.map(self.process_heterocycles_parallel, self.heterocyls, [active_params]*len(self.heterocyls), [total_fit]*len(self.heterocyls), [weights]*len(self.heterocyls))
            

        #     # Accumulate the results
        #     total_fit = sum(hydrocarbon_results) + sum(heterocycle_results)
        #old version thread+ batch
        # with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:  # 16 processes (cores)
        #     # Batch process hydrocarbons (9 files) using threads in each process
        #     hydrocarbons_batches = [self.hydrocbns[i:i+3] for i in range(0, len(self.hydrocbns), 10)]  # 3 files per batch
        #     hydrocarbon_futures = [executor.submit(self.process_hydrocarbons_batch, batch, active_params, total_fit, weights) for batch in hydrocarbons_batches]

        #     # Batch process heterocycles (50 files) using threads in each process
        #     heterocycle_batches = [self.heterocyls[i:i+10] for i in range(0, len(self.heterocyls), 20)]  # 20 files per batch
        #     heterocycle_futures = [executor.submit(self.process_heterocycles_batch, batch, active_params, total_fit, weights) for batch in heterocycle_batches]

        #     # Accumulate the fitness from hydrocarbons and heterocycles
        #     for future in concurrent.futures.as_completed(hydrocarbon_futures + heterocycle_futures):
        #         total_fit += future.result()  
        with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:
            
            hydrocarbon_futures = [executor.submit(self.process_batch, batch, active_params, total_fit, weights, 'hydrocarbons') for batch in hydrocarbon_batches]
            heterocycle_futures = [executor.submit(self.process_batch, batch, active_params, total_fit, weights, 'heterocycles') for batch in heterocycle_batches]

            # Accumulate the fitness from hydrocarbons and heterocycles
            for future in concurrent.futures.as_completed(hydrocarbon_futures + heterocycle_futures):
                total_fit += future.result()         
                
        #write_comp(calc_energies1 = 0,calc_energies2=self.comp_energies2)
        time_needed = time.perf_counter() - time_now_iter
        self.iteration += 1
        write_comp(d1_energies=dict(self.D1_energy), bright_energies=dict(self.Bright_energy), jobname=self.jobname)
        self.log_parameters(f'iteration {self.iteration} need {time_needed} seconds with total_fit {total_fit}')
        if self.opt == False:
            sys.exit()
        return total_fit
    def process_batch(self, batch, active_params, total_fit, weights, molecule_type):
        """ Process a batch of either hydrocarbons or heterocycles. """
        batch_fitness = 0
        for file in batch:
            if molecule_type == 'hydrocarbons':
                batch_fitness += self.process_hydrocarbons(file, active_params, total_fit, weights)
            elif molecule_type == 'heterocycles':
                batch_fitness += self.process_heterocycles(file, active_params, total_fit, weights)
        return batch_fitness
    ...
    def process_heterocycles_with_only_d1(self, file, energies, total_fit, weights, strng):
        ...
        # write_gnu(strng, file)
        # self.comp_energies2.update({file: [d1_calc]})
        # self.D1_energy.update({file: [d1_exp, d1_calc]})
        return fitness
